...
export class Service {
  static reorder<T>(x: number, y: number, container: T[]): T[] {
    return x < y
      ? container.slice(0, x).concat(container.slice(x + 1, y + 1), container[x], container.slice(y + 1))
      : container.slice(0, y).concat(container[x], container.slice(y, x), container.slice(x + 1));
  }
...
export class ConsultationService {
  ...
  async requestValue(requested: Variable): Promise<string> {
    return new Promise(resolve => {
      ...
      this.requestResultSub.subscribe((next: string) => resolve(next));
    });
  }
  ...
  async consult(target: Variable) {
    ...
    const conflictSet = this.store.rules.filter(
      (value: Rule) => value.conclusions.filter((conclusion: Statement) => conclusion.variable === target).length !== 0
    );
    ...
    conflictSet.forEach((rule: Rule) => {
      from(rule.premises)
        ...
        .subscribe((next: boolean) => {
          if (!!next) {
            from(rule.conclusions).forEach((conclusion: Statement) => (conflictSet[conclusion.variable.id] = conclusion.value));
          }
        });
    });
